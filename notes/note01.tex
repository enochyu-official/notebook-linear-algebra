...
{Four Lemmas on Span of a Set of Vectors}
\label{Four Lemmas on Span of a Set of Vectors}
...
\item For nonzero vectors $v_1, \ldots, v_n \in V$, $\lambda \in
\mathbb{R}$, and $1 \leq j, k \leq n$ such that $\lambda \neq 0$ and
$j \neq k$, $\Span\{ v_1, \ldots, v_k, \ldots, v_n \} = \Span\{ v_1,
\ldots, v_k + \lambda v_j, \ldots, v_n \}$. Moreover, $\Span\{ v_1,
\ldots, v_k, \ldots, v_n \} = \Span\{ v_1, \ldots, \lambda v_k,
\ldots, v_n \}$.
\end{enumerate}
\end{theorem}
...
\begin{proof}
Notice that $S \subset \Span(S)$. By the first statement,
...
+ \lambda v_j, \ldots, v_n \} \subset \Span\{ v_1, \ldots, v_k,
\ldots, v_n \}$ since all linear combination of the set of vectors on
the left can be represented as the linear combination of the set of
vectors on the right. Moreover notice that $v_k$ can be represented
as $v_k = (v_k + \lambda v_j) - \lambda v_j$ even if $k = j$.
Therefore, $v_k \in \Span\{ v_1, \ldots, v_k + \lambda v_j, \ldots,
v_n \}$ and $\Span\{ v_1, \ldots, v_k, \ldots, v_n \} \subset
\Span\{ v_1, \ldots, v_k + \lambda v_j, \ldots, v_n \}$ by statement
3. Therefore, the equations in the statement hold.
\end{proof}
...
{Finite Set of Vectors and Linear Dependence}
\label{Finite Set of Vectors and Linear Dependence}
...
\end{corollary}
\begin{proof}
By Theorem \ref{Finite Set of Vectors and Linear Dependence},
...
\begin{align*}
    V
    &= \sum_{k = 1}^n d_k v_k
    = \sum_{k = 1}^{j-1} d_k v_k + d_j v_j + \sum_{k = j+1}^n d_k v_k
...
\label{Properties of Linear Independence and Dependence}
...
\label{Basis and Linear Dependence}
...
by Theorem \ref{Basis and Linear Dependence}, any linearly
...
$n$ elements. By Theorem \ref{Basis and Linear Dependence}, since
...
\label{Unique Linear Combination}
...
\textbf{coordinates of $u$ with respect to $S$} is the list of
...
By Theorem \ref{Finite Set of Vectors and Linear Dependence}, a subset
of $S$ is linearly dependent if any only if a vector can be
...
\item More problems on basis and dimension
\end{enumerate}
\end{document}
